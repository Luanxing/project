    ...
    <div class="showtime"> 
      ...
      <ul v-for="(data,key) in datalist2" :key="key">
        <li  class="dis" v-for="(data,key) in data.list" :key="key" >
            <h2 class="times" v-show="data.startTime">{{data.showDay | Hourfilter}}:{{data.showDay | Minutesfilter}}</h2>
            <p class="hall" v-show="data.startTime">{{data.hall}}/{{data.language}}/{{data.versionDesc}}</p>
            <p class="prices" v-show="data.startTime">{{'¥' + data.price}}</p>
            <div class="buy" v-show="data.startTime">{{'购票'}}</div>
        </li>
      </ul>
    </div>
  </div>
